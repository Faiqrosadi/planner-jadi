...
def build_daily_pages(start: date, end: date, start_t: datetime, end_t: datetime, goals, j2_env: j2.Environment):
  ## MENYIAPKAN FUNCTION BUAT OTOMATISASI DI AKHIR
  ...
  # berdasarkan req, 31 desember - 1 januari (2024) = 366hari
  num_days = (end - start).days
  ## default dari jam 7 sampai jam 9
  ...
  #mulai looping
  #buat layout kiri jam 
  # dari + 0 sampai 
  # %-I for angka nya, kalo %I berari ada tambahan 0 didepan single text (01 for 1)
  #%p lambang AM or PM
  ...
  #repeat sampe setahun 1 page 
  for i in range(num_days + 1):
    cur_date = start + timedelta(days=i)

    if goals['journal'] > 0:
      ## get data date (ISO) and then ditarik berdasarkan format yang ditentukan 
      ## kenapa di tamplate malah variabel yang dipake dates instead of cur_date
      journal_link = cur_date.strftime('%Y-%m-%d-journal')

    ## masih mikir buat selang seling page nya 
...
if __name__ == "__main__":
  ...
  #   pages.extend(build_weekly_pages(start_date, end_date, start_time, end_time, env).values())
...
#   parser.add_argument('--journals-per-page', default=4, type=int, choices=[0, 1, 2, 4])
#   parser.add_argument('--dark-mode', action=BooleanOptionalAction, default=False)
